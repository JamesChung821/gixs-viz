...
# GISAXS
# INPUT_PATH = r"G:\Other computers\我的 筆記型電腦 MSI\Research data\SSID\202411\20241104 CMS AE\saxs\analysis\b59-03-CrCuNiCr-AE_afterAE"
# CONFIG_FILE = r"G:\Other computers\我的 筆記型電腦 MSI\Research data\SSID\202411\20241104 CMS AE\saxs\analysis\b59-03-CrCuNiCr-AE_afterAE\Plot\b59-03-CrCuNiCr-afterAE-SAXS.ini"

# GIWAXS
INPUT_PATH = r"G:\Other computers\我的 筆記型電腦 MSI\Research data\SSID\202411\20241104 CMS AE\maxs\analysis\b58-04-ScVMnSc-AE_afterAE-ks13-tm0d025"
CONFIG_FILE = r"G:\Other computers\我的 筆記型電腦 MSI\Research data\SSID\202411\20241104 CMS AE\maxs\analysis\b58-04-ScVMnSc-AE_afterAE-ks13-tm0d025\b58-04-ScVMnSc-afterAE.ini"
...
def gisaxs_plot(q_and_I_list, mode='Intensity', xrange=(0.004, 0.1), yrange=(0, 120)):
    ...
    for index in SAMPLE_LIST:
        ...
        elif mode == 'Kratky plot':
            plt.plot(x, x**2*y + OFFSET * increment, linewidth=3, color=CMAP(color_idx[increment + COLOR_INCREMENT]),
                     label=plot_label)  # full info
    ...
    if mode == 'Guinier':
        ...
        plt.xlabel('$\mathregular{q_r}$ ($\mathregular{\AA}^{-1}$)', fontsize=20)
        plt.ylabel('$\mathregular{q^2I(q)}$', fontsize=20, labelpad=10)
        plt.xscale('log')
        plt.yscale('log')
    else:
        plt.xlabel('$\mathregular{q_r}$ ($\mathregular{\AA}^{-1}$)', fontsize=20)
...
def out_file(q, intensity, filename):
    ...
    base_name = filename[:filename.find(FILENAME_KEYWORD)+FILENAME_KEYWORD_OFFSET]
    short_filename = f"{base_name}_{parts[SCANID_INDEX-3]}_{parts[SCANID_INDEX-2]}_{parts[SCANID_INDEX-1]}_{parts[SCANID_INDEX]}_q.xy" # <-----Please check your file name keyword and offset-----
    ...
    if PATTERN=='.xy':
        ...
        short_filename_png = f"{base_name}_{parts[SCANID_INDEX-3]}_{parts[SCANID_INDEX-2]}_{parts[SCANID_INDEX-1]}_{parts[SCANID_INDEX]}.png" # <-----Please check your file name keyword and offset-----
        ...
        plt.close()  # This prevents the figure from being displayed
        print(f"Plot saved successfully at: {output_file_png}")

    # Shorten the filename for 2theta output
    parts = filename.split('_')
    base_name = filename[:filename.find(FILENAME_KEYWORD)+FILENAME_KEYWORD_OFFSET]
    short_filename = f"{base_name}_{parts[SCANID_INDEX-3]}_{parts[SCANID_INDEX-2]}_{parts[SCANID_INDEX-1]}_{parts[SCANID_INDEX]}_tth.xy" # <-----Please check your file name keyword and offset-----
